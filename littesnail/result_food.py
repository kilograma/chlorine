...
class ResultFood(Result):
	...
	def get_result(self):
		...
		if queryLine.startswith("美食 "):
			queryLine = queryLine[len("美食 "):]
		self.queryLine_untokenized = queryLine
		queryLine = word_seg_for_search(queryLine)
		results_title = search_index(queryLine=queryLine.decode('UTF-8'), query_field="title", N=20)
		id_set = set()
		...
		results_content = search_index(queryLine=queryLine.decode('UTF-8'), query_field="content", N=20)
	...
	def to_string(self):
		...
		if len(self.match_results) == 0:
			self.result_str = "我真傻，真的，我单知道雪天里野兽在深山里没有食吃....\n竟没找到包含%s的美食..\n\n小Tips:先用简单的词(比如茄子)搜索，然后根据提示逐步扩大搜索范围.\n"%self.queryLine
		else:
			self.result_str = "菜谱里竟然没有菜和您要的菜完美重名!要不扩大一下搜索范围，例如:\n\n"
			cnt = 0
